...
export default function CategoriesClient({
  ...
  return (
    <div className="space-y-8 pb-20 font-sans">
      {/* Header */}
      <header className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 sm:gap-6 border-b border-[#B71C1C]/5 pb-4 sm:pb-6">
        <div>
          <h1 className="text-2xl sm:text-3xl md:text-4xl font-serif font-black text-[#B71C1C] tracking-tight leading-tight">
            Category Management
          </h1>
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center py-20">
          <div className="w-12 h-12 border-4 border-[#2E7D32] border-t-transparent rounded-full animate-spin mb-6" />
            ...
            <div className="lg:col-span-5 space-y-6">
              <div className="bg-white p-6 rounded-[24px] shadow-sm border border-gray-100">
                <div className="flex justify-between items-center mb-6">
                  <h3 className="text-sm font-black uppercase tracking-widest text-[#B71C1C]">Categories</h3>
                ...
                <div className="relative mb-6">
                  ...
                  <input
                    ...
                    className="block w-full pl-9 pr-3 py-2.5 bg-gray-50 border border-gray-100 rounded-xl text-xs font-bold text-[#B71C1C] outline-none placeholder:text-gray-400 focus:bg-white focus:border-[#2E7D32]/50 focus:ring-2 focus:ring-[#2E7D32]/10 transition-all"
                  />
                </div>
                ...
                <div className="space-y-4 mb-6">
                  <div className="flex gap-2">
                    <input
                      ...
                      className="flex-grow bg-[#F5F5F5] border-none rounded-xl px-4 py-3 outline-none focus:ring-2 focus:ring-[#2E7D32]/50 transition-shadow font-bold text-[#B71C1C] placeholder:font-medium placeholder:text-gray-400 text-sm"
                    />
                    <button
                      onClick={handleAddCategory}
                      disabled={!newCategory || !newCategoryImage || isAddingCategory}
                      className="bg-[#B71C1C] text-white p-3 rounded-xl shadow-lg active:scale-95 flex-shrink-0 disabled:opacity-50 disabled:cursor-not-allowed hover:bg-[#8E0000] transition-colors"
                    >
                      ...
                      <div key={cat._id} className="p-4 rounded-xl border border-gray-100 bg-white flex justify-between items-center hover:bg-gray-50 transition-colors">
                        <div className="flex items-center gap-3 min-w-0">
                          ...
                          <div className="min-w-0">
                            <span className="font-black text-xs sm:text-sm truncate block text-[#B71C1C]">{cat.name}</span>
            ...
            <div className="lg:col-span-7 space-y-6">
              ...
              <div className="bg-white p-6 rounded-[24px] shadow-sm border border-gray-100">
                <div className="flex items-center gap-2 mb-5">
                  <div className="w-7 h-7 rounded-lg bg-[#2E7D32]/10 flex items-center justify-center border border-[#2E7D32]/20">
                    <Tag size={14} className="text-[#2E7D32]" />
                  </div>
                  <div>
                    <h3 className="text-sm font-black uppercase tracking-widest text-[#B71C1C]">Step 1 — Create Sub-Category Pool</h3>
                ...
                <div className="flex gap-2 mb-4">
                  ...
                  <button
                    ...
                    className="bg-[#2E7D32] text-white p-3 rounded-xl shadow-lg active:scale-95 flex-shrink-0 disabled:opacity-40 hover:bg-[#1B5E20] transition-colors"
                  >
                    <Plus size={20} />
                  </button>
                </div>
                    ...
                    {pool.map((name) => (
                      <motion.button
                        ...
                        className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold transition-all border ${
                          mapName === name
                            ? "bg-[#B71C1C] text-white border-[#B71C1C] shadow-md ring-2 ring-[#B71C1C]/10"
                            : "bg-gray-50 text-[#B71C1C] border-gray-200 hover:border-[#B71C1C]/30"
                        }`}
                      >
                        {name}
                        <span
              ...
              <div className="bg-white p-6 rounded-[24px] shadow-sm border border-gray-100 overflow-visible">
                <div className="flex items-center gap-2 mb-5">
                  <div className="w-7 h-7 rounded-lg bg-[#B71C1C]/10 flex items-center justify-center border border-[#B71C1C]/20">
                    <Link2 size={14} className="text-[#B71C1C]" />
                  </div>
                  <div>
                    <h3 className="text-sm font-black uppercase tracking-widest text-[#B71C1C]">Step 2 — Map to Category</h3>
                ...
                <div className="space-y-4">
                  ...
                  <div className="p-4 rounded-xl bg-gray-50 border border-gray-100 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      {mapName ? (
                        <span className="text-xs font-black text-[#B71C1C] bg-[#2E7D32]/15 px-3 py-1 rounded-lg border border-[#2E7D32]/20">{mapName}</span>
                      ) : (
                  ...
                  <div className="relative">
                    ...
                    <button
                      ...
                      className="w-full bg-gray-50 border border-gray-100 rounded-xl px-4 py-3.5 text-left font-bold text-sm flex justify-between items-center transition-all focus:ring-2 focus:ring-[#2E7D32]/50 outline-none hover:border-gray-200"
                    >
                      <span className={selectedCatName ? "text-[#B71C1C]" : "text-gray-400 font-medium"}>
                        ...
                        >
                          {categories.map((cat) => (
                            <button
                              ...
                              className={`w-full text-left px-5 py-4 text-sm font-bold flex items-center gap-4 transition-colors border-b border-gray-50 last:border-0 ${mapCategoryId === cat._id ? "bg-[#B71C1C] text-white" : "text-[#B71C1C] hover:bg-gray-50"}`}
                            >
                  ...
                  <button
                    ...
                    className="w-full bg-[#2E7D32] text-white py-4 rounded-xl hover:bg-[#1B5E20] transition-all shadow-lg active:scale-95 font-black uppercase tracking-widest text-xs disabled:opacity-40 disabled:scale-100 flex items-center justify-center gap-2"
                  >
              ...
              <div className="bg-white p-6 rounded-[24px] shadow-sm border border-gray-100">
                <h3 className="text-sm font-black uppercase tracking-widest text-[#B71C1C] mb-6">Current Database Mappings</h3>
                {subLoading ? (
                  <div className="flex flex-col items-center py-10 opacity-40">
                    <Loader2 size={24} className="animate-spin text-[#B71C1C] mb-2" />
                  ...
                  <div className="flex flex-col items-center justify-center text-center opacity-40 py-12 border-2 border-dashed border-gray-100 rounded-2xl">
                    <Layers size={40} className="mb-3 text-[#B71C1C]" />
                    <p className="text-xs font-black uppercase tracking-widest text-[#B71C1C]">No active mappings found</p>
                      ...
                      return (
                        <div key={cat._id} className="animate-in fade-in slide-in-from-top-2 duration-300">
                          <div className="flex items-center gap-2 mb-3 bg-gray-50/80 p-2 rounded-lg border border-gray-100">
                            ...
                            <span className="text-[10px] font-black uppercase tracking-widest text-[#B71C1C] truncate">{cat.name}</span>
                            <span className="text-[9px] font-bold bg-[#2E7D32] text-white px-2 py-0.5 rounded-full ml-auto">{subs.length} items</span>
                          </div>
                          <div className="flex flex-wrap gap-2 pl-2">
                            {subs.map((sub: any) => (
                              <motion.div
                                ...
                                animate={{ opacity: 1, scale: 1 }}
                                className="flex items-center gap-1.5 bg-white border border-gray-200 rounded-lg px-3 py-1.5 text-xs font-bold text-[#B71C1C] group hover:border-[#2E7D32]/40 shadow-sm transition-all"
                              >
                                {sub.name}
                                <button
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[100] flex items-center justify-center p-4" onClick={onClose}>
          <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} onClick={(e) => e.stopPropagation()} className="bg-white rounded-[32px] p-8 w-full max-w-2xl max-h-[90vh] overflow-y-auto overscroll-contain shadow-2xl custom-scrollbar relative">
            <div className="flex items-center justify-between mb-8">
              <h2 className="text-2xl font-serif font-black text-[#B71C1C]">Edit Category</h2>
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-2">
                  ...
                  <input 
                    ...
                    className="w-full bg-[#F5F5F5] border-none rounded-xl px-4 py-3.5 outline-none focus:ring-2 focus:ring-[#2E7D32]/50 font-bold text-[#B71C1C]"
                  />
                </div>
                <div className="space-y-2">
                  <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest ml-1">Slug</label>
                  <input type="text" value={slug} onChange={(e) => setSlug(e.target.value)} className="w-full bg-[#F5F5F5] border-none rounded-xl px-4 py-3.5 outline-none focus:ring-2 focus:ring-[#2E7D32]/50 font-bold text-[#B71C1C]" />
                </div>
              </div>
              <div className="space-y-2">
                <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest ml-1">Description</label>
                <textarea value={description} onChange={(e) => setDescription(e.target.value)} className="w-full bg-[#F5F5F5] border-none rounded-xl px-4 py-3.5 outline-none focus:ring-2 focus:ring-[#2E7D32]/50 font-bold text-[#B71C1C] min-h-[100px] resize-none" />
              </div>
              <div className="space-y-2">
              ...
              <div className="flex gap-4 pt-4">
                ...
                <button onClick={onSave} className="flex-1 bg-[#B71C1C] text-white py-4 rounded-xl font-bold uppercase tracking-widest text-xs hover:bg-[#8E0000] shadow-lg transition-colors flex items-center justify-center gap-2"><Save size={16} /> Update Changes</button>
              </div>
            </div>
          </motion.div>
        </motion.div>
