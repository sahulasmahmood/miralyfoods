      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 sm:gap-6 bg-white p-5 sm:p-10 rounded-[1.5rem] sm:rounded-[3rem] border border-gray-100 shadow-sm transition-all">
        <div className="min-w-0">
          <h1 className="text-xl sm:text-3xl lg:text-4xl font-serif font-black text-[#007D71] leading-none">
            Product <span className="text-[#C4743F] italic">Vault</span>
          </h1>
        ...
        <button
          ...
          className="w-full sm:w-auto bg-[#007D71] text-white px-8 py-3.5 sm:py-4 rounded-xl sm:rounded-2xl font-black shadow-lg shadow-primary/20 hover:bg-[#025D53] transition-all flex items-center justify-center gap-2 active:scale-95 text-[10px] sm:text-xs uppercase tracking-widest"
        >
          <Plus size={18} />
        ...
        <div className="flex gap-2 sm:gap-3">
          <button
            ...
            className={`flex-1 sm:flex-none flex items-center justify-center gap-2 px-4 py-3 sm:py-4 rounded-xl font-black uppercase tracking-widest text-[9px] sm:text-[10px] transition-all touch-manipulation shadow-sm ${
              showFilters
                ? "bg-[#007D71] text-white shadow-lg shadow-primary/20"
                : "bg-white border border-gray-100 text-gray-400 hover:text-[#007D71]"
            }`}
          >
            <Filter size={14} /> <span>Show Filter</span>
            {selectedCategory !== "all" && (
              <span className="w-1.5 h-1.5 bg-[#C4743F] rounded-full animate-pulse" />
            )}
          </button>

          <Link
            href="/admin/uom"
            className="hidden sm:flex items-center justify-center gap-2 px-6 py-4 bg-white border border-gray-100 rounded-2xl font-black text-gray-400 hover:text-[#007D71] transition-all shadow-sm uppercase tracking-widest text-[10px]"
          >
          ...
          >
            <div className="bg-white rounded-[20px] p-6 border border-gray-100 shadow-sm">
              <div className="flex items-center justify-between mb-6">
                <h3 className="text-sm font-black uppercase tracking-widest text-[#007D71]">
                  Filter Products
                </h3>
                <button
                  onClick={() => {
                  ...
                  className="text-xs font-bold text-[#C4743F] hover:underline uppercase tracking-wider focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:outline-none touch-manipulation rounded"
                >
                  Clear All
                </button>
              </div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    <button
                      ...
                      className={`px-4 py-2 rounded-xl text-xs font-bold uppercase tracking-wider transition-colors focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:outline-none touch-manipulation ${
                        selectedCategory === "all"
                          ? "bg-[#007D71] text-white shadow-md"
              ...
              {selectedCategory !== "all" && (
                <div className="mt-6 pt-6 border-t border-gray-100">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {selectedCategory !== "all" && (
                      <span className="px-3 py-1 bg-[#F5F5F5] text-[#007D71] text-xs font-bold rounded-full flex items-center gap-2">
      ...
      <div className="space-y-4">
        {loading ? (
          <div className="flex flex-col items-center justify-center py-20">
            <div className="w-16 h-16 border-4 border-[#C4743F] border-t-transparent rounded-full animate-spin mb-6" />
          ...
          <div className="bg-white rounded-[2rem] p-12 text-center border-2 border-dashed border-gray-100">
            ...
            <h3 className="text-xl font-serif font-bold text-[#007D71] mb-2">
              No Products Found
            </h3>
            ...
            <button
              ...
              className="text-[#C4743F] font-bold hover:underline"
            >
              Clear Search
            </button>
          </div>
            ...
            {paginatedProducts.map((p, i) => {
              ...
              return (
                <motion.div
                  ...
                  transition={{ delay: i * 0.05 }}
                  className="group bg-white rounded-2xl sm:rounded-[20px] p-3 sm:p-4 border border-gray-100 hover:border-[#C4743F]/30 hover:shadow-lg hover:shadow-[#C4743F]/5 transition-all duration-300 flex flex-row items-center gap-3 sm:gap-6 relative overflow-hidden focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:outline-none touch-manipulation"
                >
                  {/* Decoration Line */}
                  <div className="absolute left-0 top-0 bottom-0 w-1 bg-[#007D71] opacity-0 group-hover:opacity-100 transition-opacity" />
                  ...
                  <div className="flex-grow flex flex-col md:flex-row md:items-center justify-between gap-4 min-w-0">
                    ...
                    <div className="flex-grow min-w-0">
                      <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-2 mb-1 min-w-0">
                        <h3 className="font-bold text-sm sm:text-lg text-[#007D71] leading-tight truncate">
                          {p.name}
                        </h3>
                        <span className="px-2 py-0.5 bg-[#F5F5F5] text-[#007D71]/60 text-[8px] sm:text-[10px] font-black uppercase tracking-widest rounded-full w-fit border border-[#007D71]/5 shrink-0">
                          {p.category}
                        </span>
                      </div>
                    ...
                    <div className="flex items-center justify-between sm:justify-end gap-4 sm:gap-8 lg:gap-12 shrink-0 pr-1">
                      ...
                      <div className="flex gap-1 sm:gap-2 shrink-0">
                        <button
                          ...
                          className="w-8 h-8 sm:w-10 sm:h-10 rounded-full flex items-center justify-center text-gray-400 hover:text-[#007D71] hover:bg-[#F5F5F5] transition-colors border border-transparent focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:outline-none touch-manipulation"
                        >
                          <Edit2
                            size={14}
