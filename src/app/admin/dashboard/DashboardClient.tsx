...
const COLORS = ["#007D71", "#C4743F", "#3D5A56", "#025D53", "#A85D30"];
  ...
  if (loading && !data) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh]">
        <div className="w-16 h-16 border-4 border-[#007D71] border-t-[#C4743F] rounded-full animate-spin mb-4" />
        <p className="text-[#007D71] font-black uppercase tracking-widest text-[10px]">
    ...
    >
        ...
        <div className="bg-white p-5 sm:p-10 rounded-[1.5rem] sm:rounded-[3rem] shadow-sm border border-gray-100 flex flex-col lg:flex-row justify-between lg:items-center gap-4 sm:gap-6">
          <div className="min-w-0">
            <motion.h1
              ...
              className="text-xl sm:text-3xl lg:text-5xl font-serif font-black text-[#007D71] leading-none"
            >
              Dashboard
            </motion.h1>
            <motion.p
            ...
            >
              <Zap
                size={14}
                className="text-[#C4743F] fill-[#C4743F] shrink-0"
              />
              Live overview for{" "}
              <span className="text-[#007D71] font-black underline decoration-[#C4743F]/50">
                Miraly Foods
              </span>
            </motion.p>
          </div>
            ...
            {(["today", "week", "month"] as const).map((r) => (
              <button
                ...
                className={`flex-1 sm:flex-none px-4 sm:px-8 py-2.5 sm:py-3.5 rounded-lg sm:rounded-xl text-[9px] sm:text-[10px] font-black uppercase tracking-[0.1em] sm:tracking-[0.2em] transition-all touch-manipulation whitespace-nowrap ${
                  dateRange === r
                    ? "bg-[#007D71] text-white shadow-lg shadow-primary/20 scale-[1.02]"
                    : "text-gray-400 hover:text-[#007D71] hover:bg-white"
                }`}
              >
                {r}
              </button>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4 sm:gap-6">
        ...
        >
          <div className="absolute top-0 right-0 p-4 sm:p-8 opacity-5 group-hover:scale-110 transition-transform">
            <TrendingUp
              size={60}
              className="text-[#007D71] sm:w-[80px] sm:h-[80px]"
            />
          </div>
          <div className="relative z-10 h-full flex flex-col justify-between">
            <div className="flex justify-between items-start mb-4 sm:mb-6">
              <div className="p-2.5 sm:p-3 bg-green-50 text-[#007D71] rounded-xl sm:rounded-2xl">
        ...
        >
          <div className="absolute top-0 right-0 p-4 sm:p-8 opacity-5 group-hover:scale-110 transition-transform">
            <ShoppingBag
              size={60}
              className="text-[#C4743F] sm:w-[80px] sm:h-[80px]"
            />
          </div>
          <div className="relative z-10 h-full flex flex-col justify-between">
            <div className="flex justify-between items-start mb-4 sm:mb-6">
              <div className="p-2.5 sm:p-3 bg-amber-50 text-[#C4743F] rounded-xl sm:rounded-2xl">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        >
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3 mb-6 md:mb-10">
            <div>
              <h3 className="text-lg md:text-2xl font-serif font-black text-[#007D71] text-balance">
                Revenue Velocity
              </h3>
            ...
            <div className="flex items-center gap-4">
              <div className="flex items-center gap-2">
                <div className="w-2 h-2 rounded-full bg-[#007D71]" />
              ...
              <AreaChart data={data.salesTrend}>
                <defs>
                  <linearGradient id="colorRev" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="5%" stopColor="#007D71" stopOpacity={0.15} />
                    <stop offset="95%" stopColor="#007D71" stopOpacity={0.01} />
                  </linearGradient>
                </defs>
                <CartesianGrid
                  strokeDasharray="5 5"
                ...
                <RechartsTooltip
                  ...
                  content={({ active, payload }) => {
                    if (active && payload && payload.length) {
                      return (
                        <div className="bg-[#007D71] text-white p-5 rounded-3xl shadow-2xl border border-white/10 backdrop-blur-xl">
                          <p className="text-[9px] font-black uppercase tracking-[0.2em] text-[#C4743F] mb-2">
              ...
              <PieChart>
                ...
                <RechartsTooltip
                  content={({ active, payload }) => {
                    if (active && payload && payload.length) {
                      return (
                        <div className="bg-[#007D71] text-white px-4 py-2 rounded-2xl shadow-xl font-black text-[10px] uppercase tracking-widest">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        >
          <div className="flex justify-between items-center mb-8">
            <div>
              <h3 className="text-xl md:text-2xl font-serif font-black text-[#007D71] text-balance">
                Signature Items
              </h3>
            ...
            {data.topProducts.map((product, i) => (
              <div key={product._id} className="group relative">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-xs font-black text-[#007D71] uppercase tracking-tight">
                    {product.name}
                  </span>
                ...
                <div className="h-2 w-full bg-gray-50 rounded-full overflow-hidden">
                  <motion.div
                    ...
                    className="h-full bg-gradient-to-r from-[#007D71] to-[#C4743F] rounded-full"
                  />
                </div>
              </div>
            ))}
      ...
      >
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3 mb-6 md:mb-10">
          ...
          <Link
            href="/admin/orders"
            className="group flex items-center gap-2 px-6 py-2.5 bg-gray-50 hover:bg-[#007D71] text-[#007D71] hover:text-white rounded-2xl transition-colors duration-300 font-black text-[10px] uppercase tracking-widest self-start sm:self-auto"
          >
            Full Ledger
            <ChevronRight
              size={14}
          ...
          <table className="w-full min-w-[700px]">
            ...
            <tbody className="divide-y divide-gray-50">
              {data.recentOrders.map((order) => (
                <tr
                  key={order._id}
                  className="group hover:bg-[#C4743F]/5 transition-colors"
                >
                  <td className="py-6 pl-4">
                    <div className="flex items-center gap-4">
                      <div className="w-10 h-10 rounded-2xl bg-gradient-to-br from-gray-50 to-gray-100 flex items-center justify-center text-[#007D71] text-xs font-black border border-gray-200">
                        {order.shippingAddress?.fullName?.[0] || "G"}
                      </div>
                      <div>
                        <p className="text-sm font-black text-[#007D71] tracking-tight">
                  ...
                  <td className="py-6 text-right pr-4">
                    <Link
                      ...
                      className="p-2 text-gray-300 hover:text-[#007D71] hover:bg-gray-50 rounded-xl transition-colors inline-block"
                    >
            ...
            >
              <div className="flex justify-between items-start">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-xl bg-white border border-gray-200 flex items-center justify-center text-[#007D71] font-black text-xs">
                    {order.shippingAddress?.fullName?.[0] || "G"}
                  </div>
                  <div>
                    <h4 className="text-xs font-black text-[#007D71] leading-none mb-1">
                      {order.shippingAddress?.fullName || "Guest"}
                    </h4>
                ...
                <div className="flex flex-col items-end">
                  <p className="text-sm font-black text-[#007D71] leading-none mb-1">
                    {formatCurrency(order.totalPrice)}
                  </p>
              ...
              <div className="flex gap-2 pt-2 border-t border-gray-100/50">
                <Link
                  href={`/admin/orders?id=${order._id}`}
                  className="flex-1 py-2.5 bg-white border border-gray-200 rounded-xl flex items-center justify-center gap-2 text-[9px] font-black uppercase tracking-widest text-[#007D71] active:scale-95 transition-all"
                >
                  <Eye size={12} />
                  View Track
                </Link>
